...
// visits could otherwise open ws://localhost:<port> and drive it drive-by. Accept ONLY the cockpit's
// own origin (loopback host AND our exact port) or no Origin at all (non-browser clients: CLI, tests).
// Pinning the port matters: another local server (a dev server with a reflected XSS, a docs preview)
// is a different origin and must not be able to drive us just because it is also on localhost.
function originAllowed(origin) {
  if (!origin) return true; // non-browser client (no Origin header); browsers always send one
  try {
    const u = new URL(origin);
    const hostOk = u.hostname === "localhost" || u.hostname === "127.0.0.1" || u.hostname === "::1";
    const portOk = String(u.port || (u.protocol === "https:" ? "443" : "80")) === String(PORT);
    return hostOk && portOk;
  } catch { return false; }
}
const wss = new WebSocketServer({ server, verifyClient: (info) => originAllowed(info.origin || info.req?.headers?.origin) });
wss.on("error", (e) => console.error("[cockpit] wss error:", String(e?.message || e)));
// Last-resort guards: a stray rejection/throw from a socket race, a child-process event, or an ACP
// frame must NEVER exit the process (that is what surfaced to the user as constant failures).
process.on("unhandledRejection", (e) => console.error("[cockpit] unhandledRejection:", String(e?.message || e)));
process.on("uncaughtException", (e) => console.error("[cockpit] uncaughtException:", String(e?.stack || e?.message || e)));

// Base serve config (bundled catalog models). The effective config the serve uses is generated by
// models.buildServeConfig() = base + any local GGUFs the user added, so it points at both.
...
function rebuildServeConfig() { serve.configPath = models.buildServeConfig(BASE_SERVE_CONFIG); }
...
// The AGENT's model runs through OpenClaw's @qvac/openclaw-plugin serve (separate from the cockpit
// serve above). These are OpenClaw catalog ids passed to `setup.provider`. Tool-heavy work (skills,
// cron, messaging) wants a capable model - a 4B/9B often loops on tool discovery instead of acting.
const AGENT_MODELS = [
  { id: "qwen3.5-4b", label: "Qwen3.5 4B (fast, weak at tools)" },
  { id: "qwen3.5-9b", label: "Qwen3.5 9B (balanced)" },
  { id: "qwen3.6-35b-a3b", label: "Qwen3.6 35B-A3B (recommended for skills/tools)" },
...
wss.on("connection", (ws) => {
  ...
  // A `ws` socket is an EventEmitter that throws on an unhandled 'error' (ECONNRESET, a tab killed
  // mid-frame, a half-open localhost TCP). Without this listener that error is uncaught and takes the
  // WHOLE server down - the single most likely cause of "the cockpit keeps dying". Swallow + drop it.
  ...
  ws.on("message", async (buf, isBinary) => {
    ...
    try {
      if (msg.type === "chat") {
        ...
        reply(true, { channels: openclaw.channels(), fields: openclaw.CHANNEL_FIELDS });
      ...
      } else if (msg.type === "channels.configure") {
        // In-app channel setup: the user pasted a token; write it to OpenClaw config + enable.
        // Do NOT log msg.fields (contains the token). The reply carries only redacted channel state.
        const r = await openclaw.configureChannel(msg.id, msg.fields || {}, { enable: msg.enable !== false });
        if (!r.ok) return reply(false, null, r.error);
        reply(true, { channels: openclaw.channels() });
        ...
        reply(true, {
          ...
          model: serve.model, models: modelChoicesNow(), serveState: serve.state,
          modelsFolder: models.getModelsFolder(), localModels: models.getLocalModels(),
          ...
          // The AGENT's model (via the @qvac/openclaw-plugin serve) is distinct from the cockpit's own
          // serve model above. governance().model is agents.defaults.model.primary (e.g. "qvac/qwen3.5-9b").
          agentModel: openclaw.governance().model,
        ...
        try {
          ...
          // Write BOTH the top-level and the agent-scoped keys: governance() reads agents.defaults.tools.*
          // first (falling back to top-level), and it is unclear which OpenClaw honors for the main agent,
          // so setting both keeps the read, the UI, and the agent's real policy consistent (review P1).
          if (msg.toolProfile) {
            let r = await openclaw.configSet("tools.profile", msg.toolProfile); if (!r.ok) return reply(false, null, r.error);
            r = await openclaw.configSet("agents.defaults.tools.profile", msg.toolProfile); if (!r.ok) return reply(false, null, r.error);
          }
          if (msg.execAsk) {
            let r = await openclaw.configSet("tools.exec.ask", msg.execAsk); if (!r.ok) return reply(false, null, r.error);
            r = await openclaw.configSet("agents.defaults.tools.exec.ask", msg.execAsk); if (!r.ok) return reply(false, null, r.error);
          }
          if (typeof msg.lean === "boolean") {
            const r = await openclaw.setLeanMode(msg.lean); if (!r.ok) return reply(false, null, r.error);
            try { acp?.stop(); } catch { /* */ } acp = null; connecting = null; // reconnect so the new tool set takes effect
            broadcast({ type: "agentState", state: "down", code: "lean-change" });
          }
        ...
        try {
          ...
          // changing the agent model means the plugin serve must respawn on it - drop the ACP session
          // so the next turn reconnects to the new model (harmless during first-run when acp is null).
          try { acp?.stop(); } catch { /* */ } acp = null; connecting = null;
          broadcast({ type: "agentState", state: "down", code: "agent-model-change" });
          reply(true, { status: st, governance: openclaw.governance() });
        }
        finally { setupBusy = false; }
        ...
        try { reply(true, models.scanGgufFolder(msg.path)); } catch (e) { reply(false, null, String(e?.message || e).slice(0, 200)); }
      } else if (msg.type === "models.add") {
        try { const m = models.addLocalModel({ path: msg.path }); rebuildServeConfig(); reply(true, { added: m, localModels: models.getLocalModels(), models: modelChoicesNow() }); }
        catch (e) { reply(false, null, String(e?.message || e).slice(0, 200)); }
      } else if (msg.type === "models.remove") {
        models.removeLocalModel(msg.alias); rebuildServeConfig();
        reply(true, { localModels: models.getLocalModels(), models: modelChoicesNow() });
